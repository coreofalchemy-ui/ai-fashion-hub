...
import { DevicePreviewSelector, DeviceType, DEVICE_CONFIGS } from './components/DevicePreviewSelector';
...
export const DetailGeneratorApp: React.FC = () => {
  ...
  const handleDownload = async () => {
    ...
    if (elementToCapture) {
      const elementsToHide = [
        elementToCapture.querySelector('#filter-preset-bar-wrapper'),
        elementToCapture.querySelector('#product-enhancement-bar-wrapper'),
        ...elementToCapture.querySelectorAll('.image-control-overlay')
  ...
  const handleDownloadJpg = async () => {
    ...
    if (elementToCapture) {
      ...
      try {
        elementToCapture.style.width = '1000px';
        elementToCapture.style.height = 'auto';
        ...
        const link = document.createElement('a');
        link.download = 'product-page.jpg';
        link.href = dataUrl;
        link.click();
      } catch (err) {
        console.error('Could not download image', err);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#FAFAFA] font-primary overflow-hidden">
      <Navigation />

      <div className="flex flex-col flex-1 bg-gray-50 text-gray-800 font-sans selection:bg-blue-100 selection:text-blue-900 overflow-hidden">
        {screen === 'result' && (
          <Header
        ...
        <main className="flex-grow overflow-hidden relative">
          ...
          {screen === 'result' && generatedData && (
            <div className="flex h-full animate-fade-in">
              ...
              <div className="w-[380px] flex-shrink-0 border-r border-gray-200 bg-white h-full overflow-y-auto z-10 shadow-xl hidden md:block">
                <AdjustmentPanel
                  data={generatedData}
              ...
              {/* Main Preview Area */}
              <div className="flex-grow h-full bg-gray-50 overflow-hidden relative flex flex-col">
                {/* Device Selector */}
                ...
                <div className="flex-1 overflow-y-auto flex items-start justify-center py-8">
                  <div
                    ...
                    style={{
                      width: `${DEVICE_CONFIGS[selectedDevice].width}px`,
                    }}
                  >
                    <div
                      className="device-screen"
                      style={{
                        height: `${DEVICE_CONFIGS[selectedDevice].height}px`,
                      }}
                    >
                      <PreviewPanel
                        data={generatedData}
                        ...
                        onRegenerateSingleProduct={async (index) => {
                          ...
                          try {
                            ...
                            alert(getFriendlyErrorMessage(e, "보정 실패"));
                          } finally {
                            setSingleProductEnhanceLoading(null);
                          }
                        }}
        ...
        <Footer isOnDressingScreen={screen === 'result'} />
      </div>
    </div>
  );
};
