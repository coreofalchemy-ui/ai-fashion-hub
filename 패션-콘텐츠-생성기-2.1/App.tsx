...
const App: React.FC = () => {
  ...
  const tabStyles = {
    inactive: "border-transparent text-gray-400 hover:text-white hover:border-gray-500",
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans">
        ...
        <div className="mb-8 border-b border-gray-700">
          <nav className="-mb-px flex justify-center space-x-8" aria-label="Tabs">
            <button
              ...
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-lg ${activeTab === 'content' ? tabStyles.active : tabStyles.inactive}`}
            >
              콘텐츠 생성하기
            </button>
            <button
              onClick={() => setActiveTab('pose')}
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-lg ${activeTab === 'pose' ? tabStyles.active : tabStyles.inactive}`}
            >
              자세 변경
            </button>
          </nav>
      ...
      <footer className="text-center py-6 text-gray-500 text-sm">
        <p>Gemini 기반. 패션의 미래를 위해 디자인되었습니다.</p>
      </footer>
    </div>
  );
};
